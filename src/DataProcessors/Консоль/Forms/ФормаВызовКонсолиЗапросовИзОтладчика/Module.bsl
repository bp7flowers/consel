...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок=СтрШаблон(НСтр("ru='Код вызова обработки %1 %2 из отладчика.'"),МетаданныеОбъекта.Имя,МетаданныеОбъекта.Комментарий);
...
Процедура ПутьКФайлуКонсолиЗапросовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Оповещение = Новый ОписаниеОповещения("ПутьКФайлуКонсолиЗапросовНачалоВыбора_Завершение", ЭтотОбъект);
	Значение = Новый Структура("ПутьКФайлуНаСервереПриложения,ФильтрТипов", ПутьКФайлуКонсолиЗапросов, ".epf");
	ОткрытьФорму(ПолучитьИмяОбработки() + ".Форма.ФормаПутиКФайлуНаСервереПриложения", Значение, ЭтотОбъект, , , , Оповещение);
...
Процедура ПутьКФайлуРезультатаЗапросаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Оповещение = Новый ОписаниеОповещения("ПутьКФайлуРезультатаЗапросаНачалоВыбора_Завершение", ЭтотОбъект);
...
Процедура ПроверкаНастроекНаСервере()
	...
	Если ВариантВызоваКонсолиЗапросов = 0 И ПустаяСтрока(ПутьКФайлуКонсолиЗапросов) Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Ошибка! Не указан путь к файлу консоли запросов.'");
	...
	Если ПустаяСтрока(НазваниеПеременнойЗапроса) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Ошибка! Не указан переменная результата запроса.'");
	...
	Если ПустаяСтрока(КаталогФайлаРезультатаЗапроса) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Ошибка! Не указан каталог файла результата запроса.'");
	...
	Если ПустаяСтрока(ИмяФайлаБезРасширения) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Ошибка! Не указано имя файла.'");
	...
	Если ВариантВызоваКонсолиЗапросов = 0 Тогда
		...
		Попытка
			...
			Если ФайлОбработки.Существует() И ФайлОбработки.ЭтоФайл() Тогда
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru = 'Указанная консоль запросов существует.'");
				Сообщение.Сообщить();
			Иначе
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru = 'Ошибка! Указан некорректный путь к файлу консоли запросов.'");
				Сообщение.Поле = "ПутьКФайлуКонсолиЗапросов";
				Сообщение.Сообщить();
			КонецЕсли;
		Исключение
	...
	Если ЗначениеВФайл(ПолныйПутьКТестовомуФайлу, Данные) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Каталог файла результата запроса доступен для записи.'");
		Сообщение.Сообщить();
	Иначе
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтрШаблон(НСтр("ru = 'Ошибка! Не удалось выполнить запись тестового файла в указанный каталог: ""%1""'"),КаталогФайлаРезультатаЗапроса);
		Сообщение.Поле = "КаталогФайлаРезультатаЗапроса";
		Сообщение.Сообщить();
	КонецЕсли;
	...
	Если ФайлРезультата.Существует() Тогда
		Попытка
			...
			Если ТипЗнч(СтруктураДанных) = Тип("Структура")
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru = 'Выбран корректный файл выгрузки.'");
				Сообщение.Сообщить();
			Иначе
				Сообщение = Новый СообщениеПользователю;
		...
		Исключение
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru = 'Ошибка! Указанный файл не подходит, т.к. не является файлом выгрузки!'");
			Сообщение.Поле = "ИмяФайлаБезРасширения";
			Сообщение.Сообщить();
		КонецПопытки;
	...
	Иначе
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Имя файла корректно.'");
		Сообщение.Сообщить();
	КонецЕсли;
