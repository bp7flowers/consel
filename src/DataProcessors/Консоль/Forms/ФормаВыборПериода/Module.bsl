...
Процедура ПриОткрытии(Отказ)
	...
	Если ПривязкаПараметров.Количество() = 0 Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Отсутствуют параметры с датой!'");
		Сообщение.Сообщить();
		Отказ = Истина;
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(ИмяТекущегоПериода) Тогда
		ЭтотОбъект.ТекущийЭлемент = Элементы[ИмяТекущегоПериода];
	КонецЕсли;

	Год = Макс(Год(НачалоПериода), Год(КонецПериода));

	Год = ?(Год = 1, Год(ОбщегоНазначенияВызовСервера.ПолучитьТекущуюДатуСеанса()), Год);
...
Процедура УстановитьПериод(СмещениеГода = 0)
	...
	Год = ?(Год = 1 ИЛИ Год = 3999, Год(ОбщегоНазначенияВызовСервера.ПолучитьТекущуюДатуСеанса()), Год);
...
Процедура Ок(Команда)
	...
	Если НачалоПериода > КонецПериода Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Ошибка! Начало периода больше конца периода.'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
