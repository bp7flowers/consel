...
Процедура ВыделитьТекущуюОбласть(Команда)
	...
	ПолеТабличногоДокумента = ЭтотОбъект.Результат;
...
Функция ЯчейкаЗаполнена(тОбл)
	...
	Возврат ТипЗнч(тОбл.Расшифровка) = Тип("Структура")
		...
		ИЛИ тОбл.Расшифровка.РольЯчейки = "СтрокаСгруппированнойТаблицы"
		ИЛИ тОбл.Расшифровка.РольЯчейки = "ШапкаСгруппированнойТаблицы");
...
&НаСервере
Процедура ДобавитьИтоговуюСтрокуТаблицыНаСервере()
	...
	Элементы.Результат.ТекущаяОбласть = ЭтотОбъект.Результат.Область(ОбластьЯчеек.Низ+1, 1, ОбластьЯчеек.Низ+1, ОбластьЯчеек.Право);

КонецПроцедуры

&НаКлиенте
Процедура ДобавитьИтоговуюСтрокуТаблицы(Команда)
	...
	ДобавитьИтоговуюСтрокуТаблицыНаСервере();
...
Процедура СвернутьТаблицуНаСервере(пРезультат = Неопределено, пПараметры = Неопределено)
	...
	Для Каждого КолнкаИсточника Из ТабЗначений.Колонки Цикл
		...
		Если СтруктураКолонокСуммирования.Свойство(КолнкаИсточника.Имя) И лТекущийТипЗначения.СодержитТип(Тип("Число")) Тогда
	...
	Для Сч = 1 По ОбластьЯчеек.Право Цикл
		Результат.Область(ЧистаяСтрока + 1, Сч).Расшифровка = Новый Структура("РольЯчейки,Значение", "ШапкаСгруппированнойТаблицы", Неопределено);
	КонецЦикла;
	...
	Для Сч = 1 По СтрокВТаблице Цикл
		...
		Для Каждого Колонка Из ТабЗначений.Колонки Цикл
			...
			Результат.Область(ЧистаяСтрока + 1 + Сч, ИндексКолонки + 1).Расшифровка = Новый Структура("РольЯчейки,Значение", "СтрокаСгруппированнойТаблицы", Оформление.Значение);
	...
	Для Сч = 1 По ОбластьЯчеек.Право Цикл
		Если СтрНайти(КолонкиСуммирования, ТабЗначений.Колонки[Сч - 1].Имя) > 0 Тогда
			...
			Результат.Область(НомерСтрокиПодвала, Сч).Расшифровка = Новый Структура("РольЯчейки,Значение", "ИтогСгруппированнойТаблицы", Оформление.Значение);
			Результат.Область(НомерСтрокиПодвала, Сч).Формат = Оформление.Формат;
		Иначе
			Результат.Область(НомерСтрокиПодвала, Сч).Расшифровка = Новый Структура("РольЯчейки,Значение", "ИтогСгруппированнойТаблицы", Неопределено);
		КонецЕсли;
	...
	ЭтотОбъект.Результат.Область(ЧистаяСтрока, , НомерСтрокиПодвала + 1, ).Видимость = Истина;

	Элементы.Результат.ТекущаяОбласть = ЭтотОбъект.Результат.Область(ЧистаяСтрока + 1, 1, НомерСтрокиПодвала, ОбластьЯчеек.Право);
...
Процедура СвернутьТаблицуНаКлиенте(пРезультат = Неопределено, пПараметры = Неопределено) Экспорт
	...
	Если пПараметры.ИмяШага = "ВыбратьКолонкиСвертки" Тогда
		...
		Оповещение = Новый ОписаниеОповещения("СвернутьТаблицуНаКлиенте", ЭтотОбъект, пПараметры);

		ОткрытьФорму(ПолучитьИмяОбработки() + ".Форма.ФормаНастройкаСверткиТаблицыЗначений", ПараметрыФормы, ЭтотОбъект, ЭтотОбъект, , , Оповещение);
...
Процедура СортироватьТаблицуНаСервере(пРезультат = Неопределено, пПараметры = Неопределено)
	...
	Для Сч = 1 По СтрокВТаблице Цикл
		ТекОтсортированнаяСтрока = ТабЗначений[Сч - 1];
		Копируемое = КопияИсходнойТаблицы.Область(ТекОтсортированнаяСтрока[ИмяКолонкиID] + 1, 1, ТекОтсортированнаяСтрока[ИмяКолонкиID] + 1, КоличествоКолонок);
...
Процедура СортироватьТаблицуНаКлиенте(пРезультат = Неопределено, пПараметры = Неопределено) Экспорт
	...
	Если пПараметры.ИмяШага = "ВыбратьКолонкиСортировки" Тогда
		...
		ПараметрыФормы = Новый Структура("ОписаниеКолонок", ОписаниеКолонок);
		Оповещение = Новый ОписаниеОповещения("СортироватьТаблицуНаКлиенте", ЭтотОбъект, пПараметры);

		ОткрытьФорму(ПолучитьИмяОбработки() + ".Форма.ФормаНастройкаСортировкиТаблицыЗначений", ПараметрыФормы, ЭтотОбъект, ЭтотОбъект, , , Оповещение);
...
Процедура РезультатВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Область.Расшифровка.РольЯчейки = "ШапкаСгруппированнойТаблицы" Тогда
		СтандартнаяОбработка = Ложь;
		Возврат;
	КонецЕсли;

	Если Область.Расшифровка.РольЯчейки = "СтрокаСгруппированнойТаблицы" Тогда
		СтандартнаяОбработка = Ложь;
		Если ЗначениеЗаполнено(Область.Расшифровка.Значение) Тогда
			ПоказатьЗначение(, Область.Расшифровка.Значение);
			Состояние("Выполняется открытие элемента.",, "" + Область.Расшифровка.Значение);
	...
	Если Область.Расшифровка.РольЯчейки = "ИтогСгруппированнойТаблицы" Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьЗначение(, Область.Расшифровка.Значение);
		Возврат;
	КонецЕсли;
...
Процедура РезультатыПоискаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Элементы.Результат.ТекущаяОбласть = ЭтотОбъект.Результат.Область(ТекущиеДанные.Ячейка);
...
Процедура ПоискЗалитьФонНаСервере(ЦветФона)
	...
	Для Каждого Строка Из РезультатыПоиска Цикл
		...
		Если РежимВыделенияФонаПоиска = 0 Тогда
			Результат.Область(Строка.Ячейка).ЦветФона = ЭтотОбъект.ТекущийЭлемент.ЦветФона;
...
Процедура ПоискЗалитьФон(Команда)
	ЦветФона = ЭтотОбъект.ТекущийЭлемент.ЦветФона;
	ПоискЗалитьФонНаСервере(ЦветФона);
КонецПроцедуры
...
Процедура РежимВыделенияФонаЯчеекПоиска(Команда)
	...
	Если ЭтотОбъект.ТекущийЭлемент.Имя = "РежимВыделенияФонаЯчеекПоиска_Ячейка" Тогда
	...
	ИначеЕсли ЭтотОбъект.ТекущийЭлемент.Имя = "РежимВыделенияФонаЯчеекПоиска_Строка" Тогда
